...
def tokenize_program(code: str) -> List[Token]:
    ...
    for line_num, raw_line in enumerate(lines, start=1):
        ...
        if indent % 4 != 0 and line.strip():
            raise SyntaxError(
                f"Line {line_num}: Indentation must be a multiple of 4 spaces"
            )
        ...
        if indent_level > indent_stack[-1]:
            if indent_level != indent_stack[-1] + 1:
                raise SyntaxError(
                    f"Line {line_num}: Indentation increased by more than 1 level"
                )
        ...
        while i < len(content):
            ...
            if c == '"':
                ...
                while j < len(content):
                    ...
                    j += 1
                if j >= len(content):
                    raise SyntaxError(f"Line {line_num}: Unclosed string literal")
            ...
            # Unknown symbol
            raise SyntaxError(f"Line {line_num}, Column {col}: Unknown symbol '{c}'")
