...
def run(code):
            ...
            if '=' in line and not line.startswith(('rubuta', 'idan', 'in ba haka ba')):
                ...
                if m and m.group(1) in functions:
                    ...
                    if len(params) != len(eval_args):
                        hus_err(f"kuskure: adadin hujjoji bai dace ba -> {fname}", f"error: wrong arg count -> {fname}")
                        continue
    ...
    while i < len(lines):
        ...
        while blocks and indent <= blocks[-1]['base_indent'] and not line.startswith('in ba haka ba'):
            blocks.pop()
        ...
        if line.startswith('aiki'):
            ...
            if not sig.endswith(':'):
                hus_err("kuskure: aiki dole ya kare da ':'", "error: 'aiki' must end with ':'")
                continue
            sig = sig[:-1].strip()
            m = re.match(r'^([A-Za-z_][A-Za-z0-9_]*)\((.*)\)$', sig)
            if not m:
                hus_err("kuskure: sunan aiki mara kyau", "error: invalid function signature")
                continue
            fname = m.group(1)
        ...
        if line.startswith('idan'):
            ...
            blocks.append({'base_indent': indent, 'condition': cond, 'else_mode': False})
            continue
        ...
        if line.startswith('in ba haka ba'):
            if not line.endswith(':'):
                ...
                continue
            if not blocks:
                hus_err("kuskure: 'in ba haka ba' bai da idan kafin sa", "error: 'else' without matching 'if'")
                continue
